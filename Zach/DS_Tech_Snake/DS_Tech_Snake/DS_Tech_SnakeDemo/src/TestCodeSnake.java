/* 
 * Filename: TestCodeSnake.java
 * Author: LamHuang & ZachXing
 * Date 2020/11/22
 * Version: V2.0.0
 * Purpose: DS_Tech
 * */
...
private static Scanner sc;
   ...
   void startNewGame() {
      ...
      dir = Dir.left;
      energy = 5000-((nd-1)*700);//Can be modified
   ...
   public void run() {
      ...
      while (Thread.currentThread() == gameThread) {
         ...
         try {
            Thread.sleep(100 - score-(int)Math.pow(nd,2));//Can be modified
         ...
         if (energyUsed() || hitsWall() || hitsSnake()) {
            ...
            if (eatsTreat()) {
               score++;
               energy = 5000-(nd*700);//Can be modified
               growSnake();
            }
            moveSnake();
            addTreat();
   ...
   void addTreat() {
      if (treats.size() < 6-nd) {//Can be modified
   ...
   public static void main(String[] args) {
	   System.out.println("Please select game difficulty(From 1 to 4)");
